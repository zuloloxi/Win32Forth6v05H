// $Id: forth.c 1.1 1994/04/01 07:51:15 andrew Exp $
        ...
        if (hwnd)
        {
            ...
            if (!doingdll)
            {
                    ...
                    if (GetWindowPlacement(hwnd, &wp) && wp.showCmd != SW_SHOWMINIMIZED)
                    {
                            ...
                            sprintf(s, "%d,%d,%d,%d\0", rc.left, rc.top, rc.right, rc.bottom);
                    for (slen = 1; s[slen] != '\0'; slen++);             // find length of the 's' buffer
                    theKey = RegGetKey ();
                    RegSetValueEx (theKey, "Console", 0, REG_SZ, s, slen+1); 
                    RegCloseKey (theKey);
                    }
            }
            DestroyWindow (hwnd);
...
int load (LPSTR cmd_line)           // returns the size of the image in bytes
{
 ...
 //       strcpy (buf, cmd_line);
    ...
    else
    {
        ...
        {
        if (!(rHndl = FindResource (the_instance, "#1234", "IMAGE")))
        {
            message ("FindResource: Failed to find the Win32Forth resource or .IMG file");
            return (0);
        }
        if (!(rGlobal = LoadResource (the_instance,rHndl)))
        {
            FreeResource (rHndl);
