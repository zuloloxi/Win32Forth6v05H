(( WINVIEW.F           An Editor for Win32Forth               by Tom Zimmer

  WinView is a multiple file text editor for Win32Forth.
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Brad Eckert's colorization support                              {BE}
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

  0  64 160  palettergb new-color DKBLUE        \ lighter shade of dark blue

create  keycolors
                black ,    red ,       green ,    yellow ,
                dkblue ,   magenta ,   cyan ,     ltgray ,
                dkgray ,   ltred ,     ltgreen ,  ltyellow ,
                ltblue ,   ltmagenta , ltcyan ,   gray ,

: keyfileName   ( -- a n ) s" WINVIEW.COL" ;
...
: ParseNum      ( a n -- a' n' number ) \ parse number from a string, return 0 if not a number
...
\ COLOR ACTION NAME where color=0..255, action=0..255, name is the string name
                ...
                r/o open-file 0=
        if      to keyfile
                begin   pad 80 keyfile read-line  if 2drop EXIT then
                ...
                while   pad swap                                ( addr len )
                   ...
                   if   0x100 /mod
                        case 0 of
                             ...
                             bounds ?do i c@ keyc, loop         \ save number and string
                             r> keywords @ dup>r 'keyword !     \ place link at end of table
                             r@
                             begin   ?dup                       \ bubble up so table is sorted
                             while   dup 1- 'keyword            \ cnt idx
                                     dup 2@ >r 16bit+ count
                                            r> 16bit+ count  compare 0<
                                     if      dup>r 2@ swap r> 2!  \ swap indicies
                                     else    drop               \ finishes with sort
                                             drop 1             \ bubbling is finished
                                     then
                                     1-
                             repeat  r> 1+ keywords !
                             endof
                             1 of to ASMoptions 2drop endof     \ enable ASM options
                             3drop                              ( a n lo hi -- a )
                        endcase
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Multiple File Support Data Structure
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
\ The following code word replaces this line of Forth words.

\ @ entry# entry-#bytes * + entry-buffer +

\ CODE entry-translate ( a1 -- a2 )
\                 mov     ecx, 0 [ebx] [edi]              \ @
...
: n$entry       ( n1 -<name>- )        \ string entry, 'n1' bytes long
              ...
              1000 #entry max-lines      \ initial maximum nuber of lines
...
FALSE value editAsBinary?       \ should we edit this file as a binary file?
  ...
  64 CONSTANT BLOCKLINE-SIZE    \ the length of each line of a binary/block file
  16 CONSTANT BLOCK-LINES       \ lines in a block
1024 CONSTANT BLOCK-SIZE        \ a Forth block size

   0 value WinViewWindow        \ for forward references
   0 value WinViewToolbar       \ allow forward references to Toolbar
...
FALSE value winview-web?        \ use WinView for Web page browsing when TRUE
...
: edit-top      ( -- n1 )
                tool-bar?
                IF      StartSize: WinViewToolbar nip
                ELSE    0
                THEN    ;
...
Font vFont                      \ define a fonts for WinView to use
Font uFont
Font iFont
Font bFont
Font xFont
...
: #line!        { caddr clen cline \ cbuf$ cdiff -- }
                ...
                ELSE    lend-len 1 =                            \ are we using a line terminator?
                        ...
                        ELSE    BLOCKLINE-SIZE cbuf$ !          \ otherwise no line terminator, set length to 64
                        THEN
                THEN
...
\ Use MAXSTRING minus one since there is a count byte at the beginning of the buffer
\ that isn't accounted for any other way.
                cur-buf LCOUNT LMAXSTRING CELL- swap /string blank ;  \ add trailing blanks
...
: update-mirrors { \ open$ -- }                 \ make mirrors have same flags
                ...
                DO      i to entry#
                        ...
                        IF      dup entry-#bytes * entry-buffer +
                                ...
                                file-lines cursor-line <   \ if other copies are beyond end of file
                                IF      file-lines 1- 0max to cursor-line
                                        cursor-line Height: DocWindow CharHeight: DocWindow / - 1+ to line-cur
...
: update-mirror-browse { \ open$ browse?? edit-changed?? -- }  \ make mirror browse flags the same
                entry# entry-max > ?EXIT
                MAXSTRING LocalAlloc: open$
                cur-filename count open$ place
                entry#
...
: file-has-changed ( -- )
                ...
                IF      NULL save-minutes 60000 * SAVE_TIMER
                        Gethandle: WinViewWindow Call SetTimer drop
                THEN
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
  ...
  BLUE                 value BmarginColor       \ browse margin color
...
:M ClassInit:   ( -- )
                ClassInit: super
                sizeof(LPWinScrollInfo) to cbSize       \ init structure to the right size
                0 to TopOfWindow
                0 to Stack#
                0 to built-cursor?
                TRUE to refreshing?
                ...
                do      font-list i cells+ @ >r
                         8                Width: [ r@ ]
                        14               Height: [ r@ ]
                        s" Courier" SetFaceName: [ r> ]  \ default to Courier
                loop
                ;M
...
\ Returns the color of the string and a flag: FALSE if not found.
                ...
                0 maxkey begin ?dup while 2/ 1 under+ repeat \ position of MSB?
                1 swap lshift
        ...
        while   >r 2/ r>
                ...
                if   0< if      over + maxkey min
                        ...
                        EXIT                    \ found
                then
        repeat  2drop  false ;                  \ not found

10 value tempbase
: tbase[        ( -- )  base @ to tempbase ;
: ]tbase        ( -- )  tempbase base ! ;

: ISHEX?        ( a n -- f )
                tbase[ hex number? ]tbase nip nip ;

: IS_##?        ( a n -- f )
\ test a string (number) for VHDL's special-radix operators
\ examples: 2#0010# 16#F00F# 8#0.234e6#
                2dup 1 /string '#' scan 2 /string '#' scan s" #" compare
                if 2drop false exit then        \ expecting ?#?#
                tbase[ decimal
                '#' kparse 1-  number? nip >r dup 2 16 between r> and 0=
                if 3drop false ]tbase exit then \ invalid decimal radix
                base !  1-                      \ set the radix for enclosed number
                2dup >float
                if fdrop 2drop true ]tbase exit then \ it's a float
                number? nip nip                 \ test for number
                ]tbase ;

: IS_BIT?       ( a n -- f )
\ test for a bit string, examples: B"1001_0010"  X"FF"
                2dup 1 /string over c@ '"' = >r
                1 /string '"' scan 1 = >r c@ '"' = r> and r> and 0=
                if 2drop false exit then        \ expecting ?"???"
                over c@ upc
        case    'B' of  2 pad !  endof
                'O' of  8 pad !  endof
                'X' of 16 pad !  endof
                3drop false exit                \ invalid base
        endcase true -rot
                2 /string 1- bounds             \ checking digits  ( f . . )
        ?do     i c@ upc pad @ digit nip        \ accept digit
                i c@ '_' = or and               \ or underline
        loop    ;

: ISNUMBER?     ( a n -- f )
                -trailing 2dup number? nip nip if 2drop TRUE EXIT then  \ decimal number
                dup 3 =
        if      over count [char] ' = swap      ( a n f a' )
                char+ c@   [char] ' = and      if 2drop TRUE EXIT then  \ '?' literal
        then    2dup >float              if fdrop 2drop TRUE EXIT then  \ floating point
                over c@ '$' =  ASMoptions 4 and and
                                         if 1 /string ishex? EXIT then  \ $hex
                ASMoptions 16 and               \ VHDL?
        if      2dup IS_##?                    if 2drop TRUE EXIT then  \ ?#??#
                2dup IS_BIT?                   if 2drop TRUE EXIT then  \ ?"??"
        then
                over 2 s" 0x" compare 0=
                                         if 2 /string ishex? EXIT then  \ 0xhex
                2drop 0 ;

: laychar       ( a n -- a' n' )                \ copy character to pad
                over 1 pad +place  1 /string ;

: ParseString   ( a n -- a' n' a len )          \ string starts with '"'
\ examples: "abc" "ab""cd"
                pad off  laychar
        begin   2dup 2>r '"' scan 1 /string ( a n a' n' )
                     2r> 2 pick - pad +place   ( a n )
                over c@ '"' = over and
        while   laychar
        repeat  pad count ;
...
: typecomment   ( a n -- )                      \ assembler comments
                GREEN SetTextColor: dc          \ display comment in green
                ASMoptions 8 and                \ ";x\" enables special comment
        if      over char+ count swap c@ '\' =  ( a n c1 . )
                if      5 - 0x0F and >r
                        over 3 chars typestr 3 /string  \ 1st 3 chars in green
                        r> ColorWord            \ alternate comment color
                else    drop
                then
        then    typestr
                BLACK SetTextColor: dc ;
...
: typetext      ( a n -- )
        ...
        if      ASMoptions 1 and                \ ';' marks a comment
            ...
            then
                ASMoptions 2 and
                if      over c@ '*' =           \ '*' in column 1 marks comment
                        if  typecomment exit
                        then
                then
                ASMoptions 32 and               \ parse out string constants
            if  2dup '"' scan dup               ( a n a' n' . )
                if      tuck 2>r - (typetext)   \ type the non-string part
                        2r> ParseString
                        RED SetTextColor: dc  typestr  \ string is in red
                        BLACK SetTextColor: dc
                        (typetext)      exit    \ type stuff after the string
                else    2drop
                then
            then
        then    (typetext) ;
...
: "PutTextOut   ( a1 n1 -- )
                ...
                THEN
                \ Added the following to make viewing binaries more readable, masks out to blanks
                ...
                IF      \ expand TABs if we are browsing
                        ...
                        WHILE   2dup K_TAB scan 2dup 2>r nip -          \ scan for TAB
                                typetext \ {BE}
\                                xpixel ypixel 2over TextOut: dc         \ output text before TAB
\                                GetTextExtent: dc drop +to xpixel       \ and adjust X pixel counter
                                2r> over c@ K_TAB =                     \ if we found a TAB
                                IF                                      \ then calc pixels by TAB width
                                        tab-size xpixel the-width / tab-size 1 max mod -
                ...
                ELSE    typetext \ {BE}
\                        dup>r                                           \ save string char width
\                        xpixel ypixel 2swap TextOut: dc                 \ output the text
\                        r> the-width *  +to xpixel                      \ adjust the X pixel counter
                THEN    ;
...
: ?HTML-EXIT    ( a1 n1 a2 n2 -- a3 n3 )
                ...
                IF      2r> nip /string         \ then remove n2 chars
                ELSE    2r> 2drop
                        r>drop                  \ don't execute the rest of the HTML statment
                THEN    ;
...
: "Process&"    { adr len \ &$ -- a2 n2 }       \ process one HTML statement
                ...
                IF      2drop                   \ August 4th, 1997 tjz
                                                \ added to correct for a stack depth problem
                ...
                THEN    adr len rot /string ;   \ remove them from original string
...
HTML: "<HR SIZE=" Get#To'>' to HRSize ;         \ extract Horizontal Rule size parameter
...
\ *****************************************************************************
\ These processors need to be last, since they are short and open ended.
\ *****************************************************************************
...
: "ProcessHTML" { adr len \ html$ -- a2 n2 }    \ process one HTML statement
                ...
                over html$ CELL+ =              \ if we didn't process any, (at the end of the chain)
                IF      2drop                   
                        adr 1 "PutTextOut       \ then output 1 original character
                        1                       \ one character processed
                ELSE    nip len swap -          \ else calc the processed characters
...
: "/TTO         ( a1 n1 n2 -- a2 n3)    \ show part n2 of string a1,n1, returning remainder
                /split                  \ split string a1 n1 into two parts
                browse?
                IF      ignoreNextLine displayingLine =         \ if we are on the next display line
...
:M StartPos:    ( -- x y )
                0 StartSize: WinViewToolbar nip 1+
                ;M
        ...
        \ vertical line beside left border
                                ...
                                ?DO
                                        ...
                                        IF      0 update-flags i + c!
                                                ...
                                                >r Width: WinViewWindow xpixel + r> ypixel + 1+
                                                line-cur i + file-lines 1- >    \ set the trail color
                                                IF      LTGRAY
                                                ELSE    line-cur i + hlst hled between
                                                        highlighting? and
                                                        IF     selectBG
                                                        ELSE    trailBG
                                                        then
                                                THEN    FillArea: dc
                                                line-cur i + to displayingLine
                                                highlighting?
                                                IF      displayingLine showHighlightLine
                                                ELSE    displayingLine #line" col-cur /string
                                                        dup "/TTO 2drop
                                                THEN
                                                ruling?
                                                IF      BLACK               LineColor: dc
                                                        HRSize 1 max 12 min 0
                                                        DO      window-Lmargin  ypixel I + MoveTo: dc
                                ...
                                IF      right-edge 0=
                                        IF   page-lines?
                                             self ConsoleWindow <> and  \ show page lines if not console
                                             IF lend-len 0=
                                                ...
                                                ELSE    linesColor  LineColor: dc
                                                        80
                                                THEN    col-cur - 0max
                                                        dup>r CharWidth: self * window-lmargin +      0 MoveTo: dc
                                                        r>    CharWidth: self * window-lmargin + Height LineTo: dc
                                             THEN
                                        ELSE    LTRED        LineColor: dc
                                                right-edge col-cur - 0max
                                                CharWidth: self * window-lmargin +      0 MoveTo: dc
                                                right-edge col-cur - 0max
                                                CharWidth: self * window-lmargin + Height LineTo: dc
                                        THEN
                                THEN
                                window-lmargin 0>
...
Can't even really get it to work in WindowsNT, because SB_THUMBTRACK doesn't work for live scroll tracking

Here is an excerpt from the Win32SDK help entry for GetScrollInfo, which is hte function
yuo use to get the scroll position of the scroll bars;

"The limitation on this technique applies to real-time scrolling of a window’s contents.
An application implements real-time scrolling by processing the WM_HSCROLL or WM_VSCROLL
messages that carry the SB_THUMBTRACK notification value, thereby tracking the position
of the scroll box (thumb) as the user moves it. Unfortunately, there is no function to
retrieve the 32-bit position scroll-box position as the user moves the scroll box.
Because GetScrollInfo provides only the static position, an application can obtain only
32-bit position data before or after a scroll operation."

So, all we can do, is use veriable size scroll bars when the file contains less than
32767 lines.  Oh, well.
))
file-lines 32767 > OR   \ if we have a big file, revert to non-resizable scroll buttons
                        IF
                                ...
                                TRUE line-cur file-lines 32767 min file-lines 1 - 1 max */ 32767 min SB_VERT
...
:M On_Init:     ( -- )
                ...
                s" #" ( A CHARACTER ) GetTextExtent: screenDC to the-height to the-width
...
:M WM_SETCURSOR { hndl msg wparam lparam \ oladr ladr llen lstrt lend lcol theLine linkCnt -- res }
                ...
                IF      ClientRect.Left dup window-lmargin 2 - + between
                        IF      uparrow-cursor
                        ELSE    FALSE to html-link?
                                FALSE to on-text?
                                browse?
                                IF      cur-column to lcol
                                        0 to lend
                                        mousey CharHeight: self / line-cur + file-lines 1- min to theLine
                                        theLine #line"  to llen to ladr
                                        ladr to oladr
                                        0 to linkCnt                                    \ #found links
                                        BEGIN   html-link? 0=                           \ haven't found a link
                                                llen     7 >= and                       \ and we have enough text remaining
                                                linkCnt  3 <  and                       \ ignore more than 3 links on a line
                                                IF      ladr llen s" <A HREF" caps-search
                                                ELSE    0 0 FALSE
                                                THEN
                                        WHILE   1 +to linkCnt                           \ found one link
                                                ...
                                                2dup + 1- c@ '"' = IF 1- THEN           \ remove trailing '"'
                                                s"   Link to: " SetStatus: WinViewWindow
                                                ...
                                                ladr llen llen lremain - /string to llen to ladr
                                        REPEAT  2drop
                                        html-link? 0=
                                        IF      lcol llen <
                                                IF      oladr lcol + c@ bl <>    \ if NOT a blank
                                                        dup to on-text?
                                                ELSE    FALSE
                                                THEN    s"  " StatusString count SetStatus: WinViewWindow
                                        ELSE    TRUE
                                        THEN
                                        IF      using-Win32s?
                                                IF      hand-cursor
                                        ...
                                        THEN
                                ELSE    ibeam-cursor
                                        s"  " StatusString count SetStatus: WinViewWindow
                                THEN
                        THEN
                        1
                ELSE    drop
...
: set-console-height ( -- )
                GetSize: WinViewWindow nip drag-barV - to console-height ;
...
: minimized-EditWindow? ( -- f1 )              \ is edit window NOT showing
                drag-barV StartSize: WinViewToolbar nip char-height 2* + < ;

: unminimize-EditWindow ( -- )
                minimized-EditWindow?
                IF      GetSize: WinViewWindow nip 2/ to drag-barV
                        set-console-height
                        Refresh: WinViewWindow
                THEN    ;
...
create about-edit-msg2
                ...
                +z," implemented in WinView. Plus many new features\n"
...
: about-demo    ( -- )
                WinViewWindow Start: about-demo-dialog drop
                SetFocus: DocWindow ;
...
: set-line-pointers { textlen \ bincnt -- }
        ...
        IF      text-ptr textlen LMAXCOUNTED 1- umin 0x0D scan drop 1+ c@ 0x0A =    \ CRLF or
                textlen 0= or                                                       \ empty file
                IF      0x0D to lend-char
                        2    to lend-len
                        text-ptr textlen + 2 - w@ 0x0A0D <>     \ really 0x0D, 0x0A, cause bytes are reversed for W@
                        IF      0x0A0D text-ptr textlen + w!
                                2 +to textlen
                        THEN
                THEN
                text-ptr textlen LMAXCOUNTED umin 0x0A scan 0> swap 1- c@ 0x0D <> and     \ LF only APPLE
                IF      0x0A to lend-char
                        ...
                        THEN
                THEN
                text-ptr textlen LMAXCOUNTED 1-  umin 0x0D scan 0> swap 1+ c@ 0x0A <> and \ CR only UNIX
                IF      0x0D to lend-char
                        1    to lend-len
                        text-ptr textlen + 1 - c@ lend-char <>
                        IF      lend-char text-ptr textlen + c!
        ...
        IF      text-ptr textlen
                BEGIN   lend-char scan dup
                WHILE   lend-len /string over line-tbl line-cur cells+ !
                                                                        \ if line longer than LMAXCOUNTED chars
                        line-tbl line-cur cells+ dup @ swap cell- @ 2dup - LMAXCOUNTED 1- u>
                        IF      tuck -                                  \ convert to adr,len
                                BEGIN   BLOCKLINE-SIZE /string dup      \ treat each 64 chars as a line
                                WHILE   BEGIN   2dup swap c@ bl <> and  \ look for next blank
                                        WHILE   1 /string               \ scan till find one
                                        REPEAT  over c@ bl =            \ found a blank?
                        ...
                        THEN
                REPEAT  drop
        ELSE                            \ no line terminator, make 64 characters lines
                text-ptr textlen
                BEGIN   dup
                WHILE   BLOCKLINE-SIZE /string over line-tbl line-cur cells+ !
                        1 +to line-cur                  \ bump current line
                        1 +to file-lines
                        ...
                        THEN
                REPEAT  drop
        THEN    line-cur   1 max to line-cur
...
: >col-cursor   ( n1 -- )
                ...
                IF      cursor-col screen-cols 4 - - HPosition: DocWindow
                ELSE    cursor-col col-cur <
                        IF      cursor-col           HPosition: DocWindow
                        THEN
                THEN
                highlight-cursor
                refresh-line
...
: xsearch       { sa1 sn1 sa2 sn2 -- a3 n3 f1 }         \ case sensitive selectable
...
: -xsearch      { sadr slen fadr flen \ ffnd -- a3 n3 f1 }
                0 to ffnd
                sadr slen
                begin   fadr flen xsearch
                while   2dup to slen to sadr
                        1 /string
                        true to ffnd
                repeat  2drop sadr slen ffnd ;
...
: back-find-text-again ( -- )
                ...
                find-buf c@ 0=
                IF      TRUE to busy?
                        find-buf WinViewWindow Start: FindTextDlg
...
: find-label    { line# -- flag }       \ flag=TRUE if we found the label
                ...
                IF      r> /string                      \ remove leading "<A NAME" string
                        ...
                        drop find-buf count tuck caps-compare 0=
                                ...
                                ?DO     i .searching
                                        ...
                                        IF      i find-label
                                                IF      0=      \ invert the flag on stack
                                                        LEAVE
                                                THEN
                                        ELSE    i #line" find-buf count xsearch
                                                IF      i #line" rot - nip nip
                                                        i to-find-line
                                                        0= LEAVE
                                                ELSE    2drop
...
: back-tab      { \ hlsts hcsts hleds hceds -- }
                ...
                IF      hlst to hlsts
                        ...
                        ?DO     i hcsts _>row-col
                                get-cursor-line
                                tab-size 0
                                ?DO     cur-buf CELL+ cursor-col + c@ bl =      \ blank at cursor
...
: replace-text  ( -- )
                bitImage? ?EXIT
                browse?
                IF      find-buf WinViewWindow Start: FindTextDlg
                        dup 2 = to CaseSensitive?
                        IF      1 +to cursor-col       \ skip one character
                                _find-text-again ?beep
                        THEN
                ELSE    WinViewWindow Start: ReplaceTextDialog
                THEN    ;
...
: goto-line     { \ line$ -- }  \ move cursor to line entered in dialog
                ...
                line$ off
                BEGIN   line$ WinViewWindow Start: GotoDlg
...
: copy-text     { \ gblhndl gblptr -- } \ Copy text to Windows clipboard
                bitImage? ?EXIT
                highlighting? 0=                \ nothing is highlighted
                IF      highlight-word          \ try to highlight something
                THEN
                ...
                THEN
                wait-cursor             \ release/allocate line pointer table
                GetHandle: WinViewWindow call OpenClipboard 0=
        IF      beep
        ELSE    hlst #line" drop hcst +         \ start of data
                hled file-lines <
                ...
                call GlobalAlloc to gblhndl     \ allocate a buffer
                                                \ lock memory
                gblhndl call GlobalLock abs>rel to gblptr
...
: paste-text    { \ gblhndl gblptr tlen tadr iadr textlen -- }
                bitImage? ?EXIT
                browse? ?EXIT
                delete-highlight
                GetHandle: WinViewWindow call OpenClipboard 0=
        IF      beep
        ELSE    wait-cursor             \ release/allocate line pointer table
                get-cursor-line        \ cursor may be to right of line end
                cur-buf @ cursor-col max "LCLIP" cur-buf !  \ extend line
                put-cursor-line                                 \ if needed
                CF_TEXT call GetClipboardData ?dup
           IF   to gblhndl
                gblhndl call GlobalLock abs>rel to gblptr       \ lock memory
                gblptr 4000000 2dup 0 scan nip - to tlen to tadr
                tlen
                IF   text-length to textlen
                     ...
                     textlen tlen + start-text-size + to text-blen
                     text-blen text-ptr realloc              \ adj buffer
                     s" Failed to adjust the text buffer size" ?TerminateBox
                     to text-ptr
...
: load-colors   ( -- )
                GetHandle: WinViewWindow Start: ColorText dup c@ \ -- a1 n1
        IF      count open-keywords
        ELSE    drop beep                               \ {BE}
        THEN    refresh-screen ;

FileNewDialog ViewText "View Text File" "Forth Files (*.F)|*.F|C Files (C,CPP,etc..)|*.C;*.CPP;*.H;*.MAK;*.RC|HTML Files (*.HTM)|*.HTM|Batch Files (*.BAT)|*.BAT|All Text Files|*.C;*.CPP;*.F;*.TXT;*.SEQ;*.H;*.LST;*.RC;*.MAK|All Files (*.*)|*.*|"
...
: "open-text    { nadr nlen \ textlen open$ create$ -- }
                ...
                IF      WinViewWindow Start: DocWindow  \ then startup child window
                        Refresh: WinViewWindow
                THEN
                ...
                IF      open$ count "path-file drop open$ place
                        ...
                        IF      open$ ?defext
                                ...
                                IF      drop                    \ can't, then strip off path
                                        nadr nlen "to-pathend" "CLIP" open$ place
                                                                \ search through the Forth path
                                        open$ count "path-file drop "CLIP" open$ place
                                        open$ ?defext           \ make sure it has an extention
                ...
                IF      open$ count s" .BMP" caps-search nip nip        \ if it's a BMP
                        ...
                        ELSE                                    \ else open as text file
                                ...
                                addFileMenu
                        THEN
                ELSE    drop                            \ discard garbage handle
                                                        \ we make a NEW FILE here
                        nadr unnamed-file?
                        auto-new? or                    \ automatically make a new file
                                                        \ if it doesn't already exist
                        IF      TRUE
                        ELSE    unnamed-file count RECURSE      \ recurse, make new
                                nadr nlen "CLIP" open$ place
                                open$ count "path-file drop open$ place
                                open$ ?defext           \ make sure it has an extention
                                ...
                                WinViewMessageBox: WinViewWindow IDOK =
                        ...
                        ELSE    unnamed-file count RECURSE      \ recurse, make new
                                ...
                                EXIT                            \ and leave
                        THEN
                THEN    textlen set-line-pointers
                        set-longest-line
...
: _save-text    { \ save$ fhndl -- }    \ save the file in memory to a disk file
                ...
                edit-changed? 0= ?EXIT
                cur-filename c@ 0=
                IF      GetHandle: WinViewWindow Start: SaveText dup c@
                ...
                ELSE    drop beep
                        ...
                        MB_OK WinViewMessageBox: WinViewWindow drop
                THEN
                SetFocus: DocWindow
                ReTitle: WinViewWindow ;
...
: save-text-as  ( -- )          \ save with a new name
                ...
                IF      save-bitmap-as
                ELSE    no-highlight
                        GetHandle: WinViewWindow Start: SaveText dup c@
...
: _warn-to-save ( -- )
                ...
                MB_YESNO WinViewMessageBox: WinViewWindow
                CASE
...
: _?save-text   ( -- f1 )       \ return f1=true if we canceled
                ...
                IF      save-minutes 0>
                        ...
                        ELSE    z" Save the changes to the current file?"
                                ...
                                WinViewMessageBox: WinViewWindow   
                                CASE
...
' _?save-text is ?save-text     \ link into defered word
...
: _save-bitmap-as { \ nBits pbmi lpBits hbm  hdcMem hfile nrgbquad BitmapFileHeader save$  -- }
        ...
        GetHandle: WinViewWindow Start: SaveBitmap dup c@
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ Calculate printer font height
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

: winview_calc_font_height ( --- points_high )
                84 printer-lpi / ;

' winview_calc_font_height is calc_font_height

\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ Print Text File support
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
: print-text    { \ message$ -- }
        ...
        IF
                ...
                ?DO     i #line"
                        ...
                        IF      BEGIN   2dup s" <" search
                                WHILE   2swap 2over nip - Type: ThePrinter
                                        2dup '>' scan dup
                                        IF      2swap 2drop
                                        ELSE    2drop dup
        ...
        THEN    r> to char-width
                r> to char-height
                FALSE to direct-print? ;        \ if we enable auto-print, then use it only once.

: text-setup    ( -- )
                bitImage? ?EXIT
                GetHandle: WinViewWindow Setup: ThePrinter ;

: text-up-setup ( -- )
                bitImage? ?EXIT
                WinViewWindow Start: page-up-dialog to #pages-up
                SetFocus: DocWindow ;

\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Hyper Text linkage words
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
: "hyper-link   ( a1 n1 -- )
                s" WINVIEW.NDX" _"hyper-link ;

: "help-link    ( a1 n1 -- )
                s" HELP.NDX" _"hyper-link ;
...
: word-link     ( -- )          \ let user type in a word to link to
                ...
                hyper-buf WinViewWindow start: HyperTextDlg
                IF      0 to hyper-start
                        next-hyper-link
                THEN    SetFocus: DocWindow ;
...
: open-text     { \ open$ -- }
                ...
                >E-unminimize
                ?save-new ?EXIT
                GetHandle: WinViewWindow Start: ViewText dup c@ \ -- a1 n1
                IF      count open$ place
...
: "HTML         { htmlAdr HtmlLen \ open$ html$ name$ fhndl -- }
                ...
                >E-unminimize
                ?save-new ?EXIT
                s" InternetAttemptConnect" [ also hidden ] "find-proc 0= ?EXIT drop [ previous ]
                HAVEInternet? 0=
...
: "ViewWeb-Link ( a1 n1 -- )
                winview-web?                    \ user selected WinView for web browsing
                ?control OR
                IF      "HTML
                ELSE    GetHandle: WinViewWindow "Web-Link
                THEN    ;
...
: command-options ( -- )
                ...
                WHILE   count 1 /string                         \ remove leading char
                        ...
                        IF      bl word count                   \ pick up next word
                                2dup '.' scan nip -             \ remove trailing file extention
                                dup                             \ if name specified use it
                                IF      2dup     config-name  place
                                        s" .CFG" config-name +place
                                                  index-name  place
                                        s" .NDX"  index-name +place
                                THEN
        ...
        \ first character to decide if they match or not.
                        ...
                        IF      \ added new clipping to DEFEXT$, for new max length of 16 chars
                                2dup 1 /string DEFEXTMAX min defext$ place      \ default file extention
                                dup /string                                     \ discard remainder
                        THEN
                        ...
                        IF      2dup 1 /string number? 2drop
                                ...
                                dup /string                     \ discard remainder
                        THEN
        \ if the command was not recognized, then just display the help dialog
                        nip
                        IF      show-command-help
                                bye
                        THEN
                REPEAT  count file-to-edit$ place ;
...
: split-line    ( -- )                  \ split line at cursor
                ...
                get-cursor-line
                auto-margin?
                IF      cur-buf LCOUNT 2dup bl skip nip - nip cursor-col min
                        right-edge 0>
                ...
                cursor-line screen-rows  2 - - 0max line-cur max to line-cur
                file-has-changed
                refresh-screen ;
...
: _?wrap-word   { \ original-col wrap-col -- }
                ...
                If      all-lines: DocWindow
                        ...
                        get-cursor-line
                        cur-buf CELL+ cursor-col + 1- c@ bl <>  \ char before cursor not a blank?
                        IF      word-left
                        THEN
                        ...
                        cur-buf LCOUNT cursor-col /string bl skip nip 0<> and
                        ...
                        ELSE                                    \ back to middle of wrap word
                                cursor-line original-col wrap-col - left-margin +
                        THEN    >row-col
                THEN    ;
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ Cause text command string to be interpreted.
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ Process a carraige return from user
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ Search Files for text dialog Object
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
: "AddFile      ( a1 n1 a2 n2 line -- ) \ add file a2,n2 to file list
                ...
                sifListLen maxList 990 - <      \ should never be less than about 1500 bytes free
        IF      dup>r sifList sifListLen + !                    \ lay in line number
                "clip" 2dup
                2dup "to-pathend" nip maxName max to maxName    \ save longest filename length without path
                sifList sifListLen + cell+ place                \ lay in filename
                2swap "clip"
                sifList sifListLen + cell+ MAXSTRING + place    \ lay in search line text
        ...
        THEN    \ now resize search list if it is relatively near being full.
                ...
                sifListLen maxList 2000 - >=    \ keep lots of free space at end of search list
...
load-dialog WINVIEW     \ load the dialogs for WinView

:Object FindInFilesDlg  <Super  ModelessDialog

IDD_SEARCHINFILES winview find-dialog-id constant template
...
: save-search-list { \ lsthndl lsterr msg$ file$ -- }
                ...
                IF      total-found 1000 >
                        IF      originX 7 + originY 50 + SetOrigin: msg-window
                                ...
                                Start: msg-window
                        THEN
                        s" SEARCHLIST.TXT" r/w create-file 0=
                        IF      to lsthndl
                                0 to lsterr
                                file$ off
                                0                                               \ start at zero
...
: find-in-files ( -- )
                ...
                ELSE    WinViewWindow Start: FindInFilesDlg
                THEN    ;
...
IDD_SIZE winview find-dialog-id constant template
...
:M GetFont:     { \ csel -- a1 n1 }
                ...
                csel 0<
                IF      s" Courier"
                ELSE    textfont$ rel>abs csel LB_GETTEXT ID_TEXTFONT SendDlgItemMessage: self
...
:M GetFont/Size: ( -- )
             ...
             s" #" ( JUST A CHARACTER ) GetTextExtent: screenDC to the-height
                                                                to the-width
                        GetHandle: screenDC ReleaseDC: self
                        ReSize: WinViewWindow
                        Paint: DocWindow
                ELSE    drop
                THEN    ;M
...
: text-size     ( -- )
                bitImage? ?EXIT
                WinViewWindow Start: TextSizeDlg drop
                SetFocus: DocWindow ;
...
IDD_OPTIONS winview find-dialog-id constant template
...
\ May 6th, 1998 - 17:53 tjz added 'ELSE 2DROP' per Bruno Gauthier
                ...
                IDB_WEB        IsDlgButtonChecked: self 0<> to winview-web?
...
\                IDB_OPENPROMPT IsDlgButtonChecked: self 0<> to open-previous?
                ...
                min-tool-bar? 0=
                WinViewToolbar max-toolbar = XOR
                tool-bar? Floating: WinViewToolbar OR AND
                IF      SwitchToolBar: WinViewWindow
                THEN
                IDB_TOOLBAR    IsDlgButtonChecked: self 0<> ( -- f1 )
    ( f1 -- )   IF      StartToolBar: WinViewWindow
                ELSE    Floating: WinViewToolbar
                        IF      TRUE to tool-bar?
                        THEN    CloseToolBar: WinViewWindow
                THEN

                second-copy?     0=                             \ not the second copy of WinView
                R> show-console? <> and                         \ and console state changed
                IF      show-console?                           \  we want the console
                        IF      StartConsole: WinViewWindow
                        ELSE    CloseConsole: WinViewWindow
...
: text-options  ( -- )
                WinViewWindow Start: OptionsDlg drop
                SetFocus: DocWindow
                refresh-screen ;


\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Various Higher Level Functons
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
: sort-paragraph ( -- )         \ sort lines of a paragraph
                ...
                MB_OKCANCEL WinViewMessageBox: WinViewWindow IDOK =
                IF      >E-unminimize
                        do-sort-paragraph
                        key?
                        IF      key drop
...
: inquirebp     ( -- )          \ inquire for the value of a data item
                ...
                IF      TRUE to inq-running?
                        ...
                        IF      ed-inquire WinViewWindow Start: InquireDlg
                        ELSE    TRUE
                        THEN
...
IDD_DEBUG WINVIEW find-dialog-id constant template
...
: debug-buttons ( -- )
                WinViewWindow Start: DbgButtonsDlg
...
: debug-word    ( -- )          \ set a breakpoint on a word
                ...
                ELSE    debug-buf count -trailing nip debug-buf c!  \ no trailing bl's
                        debug-buf WinViewWindow start: DebugDlg
...
: debug-forth   ( -- )
                ...
                THEN
                IF      debug-word
                ELSE    z" Sorry, there is no instance of a\nWin32Forth application running!" zMessageBox
                THEN    ;

\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Convert a decimal number to hex in the 0x00000000 format
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
: make-hex      ( -- )
                ...
                IF      base @ >r                                       \ save the base
                        ...
                        2dup bl scan nip - 31 min hex-buf place         \ parse out first word
                        ...
                        IF      delete-highlight                        \ delete old number
                                ...
                                2 /string                               \ remove "0x" from the start
...
: edit-macro    ( -- )
                Gethandle: WinViewWindow Start: EditLog dup c@
...
: play-macro    ( -- )
                Gethandle: WinViewWindow Start: PlayLog dup c@
                IF      count "playkeys
                ELSE    drop
                THEN    SetFocus: DocWindow ;
...
: new-macro    ( -- )
                Gethandle: WinViewWindow Start: NewLog dup c@
                IF      count "new-log
                ELSE    drop
                THEN    SetFocus: DocWindow ;
...
: "AddLine      { adr len \ list$ -- }
                ...
                list$ 1+ rel>abs
                0 LB_ADDSTRING
                GetID: self SendDlgItemMessage: WinViewWindow drop
                ;

: selected-item ( -- n1 )       \ the currently selected item of list box
                0 0 LB_GETCURSEL
                GetID: self SendDlgItemMessage: WinViewWindow ; ( -- n1 )

:M GetLine:     { ndx -- a1 n1 }
                lineBuf 1+ rel>abs ndx LB_GETTEXT
                GetID: self SendDlgItemMessage: WinViewWindow drop
                0 ndx LB_GETTEXTLEN
                GetID: self SendDlgItemMessage: WinViewWindow lineBuf c!
                lineBuf count
                ;M
...
Addr: files-list to FilesList   \ initialize the object pointer for forward references
...
:M BInfo:       ( -- a1 )       \ return address of filename under cursor
                                \ in the files list box
                0 0 LB_GETTOPINDEX
                GetID: self SendDlgItemMessage: WinViewWindow   \ -- n1 index of top item
...
: load-forth-file { \ load$ file$ -- }
                ...
                MAXSTRING LocalAlloc: file$
                Gethandle: WinViewWindow start: LoadForth dup c@
                IF      count file$ place
                        show-console? 0=
...
: SaveFindStrings ( -- )
        ...
        tool-bar? and
        IF      GetCount: findComboEdit 10 min 0             \ only save first 10 find strings
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Enhance the ToolBar class for WinView
\ We need a special class, because of the extra status information that is
\ displayed in the toolbar, with the nice box around it.
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
: show-edit-mode ( -- )
                ...
                ELSE    entry# GetStack: ConsoleWindow =
                        IF      s" Forth      " BLACK                   \ console
                                SetTextColor: dc TextOut: dc
                        ELSE    entry# GetStack: ConsoleWindow >                \ other windows
                                IF      2drop
                                ELSE    edit-changed?
                                        ...
                                        ELSE    lend-len 2 =
                                                ...
                                                ELSE    lend-len 0=
                                                        IF      s" Binary/BLOCK       "                 BLUE    \ binary or block
                                                        ELSE    lend-len 1 =
                                                                IF      lend-char 0x0D =
                                                                        IF      s" Apple              " WHITE   \ Macintosh
                                                                        ELSE    s" Unix               " BLACK   \ UNIX
                                                                        THEN
                                                                ELSE    s" *UNKNOWN*          "         RED     \ error
                                                                THEN
                                                        THEN
                                                THEN
                                        THEN    SetTextColor: dc TextOut: dc
                                THEN
                        THEN
                THEN
                3 - swap 11 - swap
...
:M Float:       (  -- )
                floatBar 0=
                IF      CloseToolBar: WinViewWindow
                        TRUE to floatBar
                        WinViewWindow Start: self
                        min-tool-bar?
                        IF      StartPos: WinViewWindow 54 - SetOrigin: self
                        ELSE    StartPos: WinViewWindow 80 - SetOrigin: self
                        THEN
                        Update: self
                        TRUE to tool-bar?
                        LoadFindStrings
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Define the ToolBar for the application
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

WVToolBar max-Edit-Tool-Bar "WINVIEW.BMP"
max-Edit-Tool-Bar to max-toolbar
        3 HSpace
    22 TinyButton    Float: WinViewToolbar ;
    ...
    21 PictureButton                    save-all-text   ; \ save all files
     ...
     4 PictureButton                    close-text      ; \ close
        ...
        8 HSpace
     3 PictureButton  TRUE to direct-print? print-text  ; \ print
...
DefToolWidth DefToolSpacing + HSpace
    ...
    19 PictureButton                    text-options    ; \ Preferences
        ButtonInfo"  WinView Preferences  (Ctrl+Shft+P) "
...
WVToolBar min-Edit-Tool-Bar "WINVIEW.BMP"
        3 HSpace
    22 TinyButton    Float: WinViewToolbar ;
        ButtonInfo"  Float ToolBar toggle "
        5 HSpace
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Define the Editor popup menu for the application
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
POPUPBAR Edit-Popup-bar
    POPUP " "
        ...
        MENUITEM        "Open ToolBar"                          StartToolBar: WinViewWindow ;
        MENUSEPARATOR
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Define the Console popup menu for the application
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
POPUPBAR Console-Popup-bar
    POPUP " "
        ...
        MENUITEM        "Separate Forth Console"                CloseConsole: WinViewWindow
                                                                0 WM_FORTHIO win32forth-message ;
        MENUITEM        "Close Forth Process"                   CloseConsole: WinViewWindow
                                                                1 WM_TERMINATE win32forth-message ;
        MENUSEPARATOR
        MENUITEM        "Exit WinView"                          bye ;
ENDBAR

\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Define the ToolBar popup menus for the application
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

POPUPBAR max-ToolBar-Popup-bar
    POPUP " "
        MENUITEM        "Minimum ToolBar"       SwitchToolBar: WinViewWindow ;
        MENUITEM        "Float ToolBar toggle"          Float: WinViewToolbar ;
        MENUITEM        "Close ToolBar"          CloseToolBar: WinViewWindow ;
ENDBAR

POPUPBAR min-ToolBar-Popup-bar
    POPUP " "
        MENUITEM        "Full ToolBar"          SwitchToolBar: WinViewWindow ;
        MENUITEM        "Float ToolBar toggle"          Float: WinViewToolbar ;
        MENUITEM        "Close ToolBar"          CloseToolBar: WinViewWindow ;
ENDBAR
...
:M InsertMenuItem: { zMenuText pFunc \ iFlag text$ -- }
                MAXSTRING LocalAlloc: text$
                FALSE to iFlag                                  \ mark as not already inserted
                insertedMenus 0
                ?DO     mid[] i CELLS+ @ NULL <>                \ if the id isn't NULL
                        IF      MF_BYCOMMAND
                                MAXSTRING
                                text$ 1+ rel>abs
                                mid[] i CELLS+ @                \ use the ID to get text
                                pid Call GetMenuString text$ c!
                                zMenuText text$ count tuck CAPS-COMPARE 0= iFlag or to iFlag
                        THEN
                LOOP
                ...
                IF      insertedMenus 0=
                        IF      NextId    mid[] insertedMenus CELLS+ !  \ set the menu ID
                                ...
                                1 +to insertedMenus
                        ELSE                            \ make room for an additiona entry
...
( >>> ) MENUITEM        "Save and Load the active Forth Source \tF12" save-and-load ;
  ...
  false MENUMESSAGE     "Recent Files"
    ...
    POPUP "&Edit"
        ...
        MENUITEM        "WinView &Preferences...    \tCtrl+Shft+P"         text-options ;
        MENUSEPARATOR
        SUBMENU     "ToolBar Options"
                MENUITEM        "Open/Close ToolBar"  tool-bar? IF          CloseToolBar: WinViewWindow
                                                                ELSE        StartToolBar: WinViewWindow THEN ;
                MENUITEM        "Float/Dock ToolBar"                        Float: WinViewToolbar ;
                MENUITEM        "Full/Minimum ToolBar"                      SwitchToolBar: WinViewWindow ;
                ENDSUBMENU
...
\        MENUITEM        "Toggle &Colorization"                             toggle-color ; \ {BE}
    ...
    POPUP "&Search"
        ...
        MENUSEPARATOR
        MENUITEM        "Find &Text in Files...\tCtrl+Shft+F"              find-in-files ;
        MENUSEPARATOR
    ...
    POPUP "&Macros"
        ...
        MENUITEM        "&RePlay last Key File       \tCtrl+Shft+M or F5"  replay-macro ;
        MENUITEM        "&Repeat Key File n times..  \tCtrl+Shft+R"        WinViewWindow repeat-amacro ;
...
SYS-WARNING-OFF
  ...
  false MENUMESSAGE     "--- List Functions ---         \tForth Word"
        ...
        MENUITEM        "List of Defered &Word          \t.DEFERED"  s" .DEFERED"  "CONSOLE ;
  ...
  false MENUMESSAGE     "--- Date Functions ---         \tForth Word"
    ...
    POPUP "&Help"
        MENUITEM        "&Help me get Started!               \tF1"      s" STARTUP.TXT"  "BROWSE ;
        MENUITEM        "&Help on highlighted ANS-Forth word \tCtrl+F1" help-link ;
        MENUITEM        "&Win32Forth READ.ME"                           s" READ.ME"      "BROWSE ;
        MENUITEM        "&Win32Forth FAQ's"                             s" WIN32FOR.FAQ" "BROWSE ;
        MENUITEM        "&Commandline Help"                             show-command-help ;
        MENUSEPARATOR
        MENUITEM        "&Whats NEW in Win32Forth"                      s" WIN32FOR.NEW" "BROWSE ;
        MENUITEM        "&Previous Win32Forth Changes"                  s" WIN32FOR.PRV" "BROWSE ;
        MENUITEM        "&Utilities in Win32Forth"                      s" UTILDOC.TXT"  "BROWSE ;
        MENUITEM        "&ANS Required Documentation"                   s" ANSI.TXT"     "BROWSE ;
        MENUITEM        "&DPANS94 Document"                             s" DPANS.HTM"  "BROWSE ;
        MENUITEM        "ANS Forth Word List"                           s" DPANSF.HTM"   "BROWSE ;
        MENUITEM        "Assembler Documentation"                       s" 486ASM.TXT"   "BROWSE ;
        MENUSEPARATOR
  false MENUMESSAGE     "Web Based Resources"
        MENUSEPARATOR
        SUBMENU     "Forth Interest Group"
                MENUITEM    "Forth Home page"                   s" http://www.forth.org"                                "ViewWeb-Link ;
                MENUITEM    "FIG Membership Information"        s" http://www.forth.org/membership.html"                "ViewWeb-Link ;
                MENUITEM    "Forth Literature page"             s" http://www.forth.org/forthlit.html"                  "ViewWeb-Link ;
                MENUITEM    "Forth Compilers page"              s" http://www.forth.org/forthcomp.html"                 "ViewWeb-Link ;
                MENUITEM    "FTP access"                        s" ftp://ftp.forth.org/pub/forth"                       "ViewWeb-Link ;
                MENUITEM    "FIG UK"                            s" http://www.users.zetnet.co.uk/aborigine/forth.htm"   "ViewWeb-Link ;
                ENDSUBMENU
        SUBMENU     "Forth Web pages"
          false MENUMESSAGE      "Introductory"
                MENUSEPARATOR
                MENUITEM    "Leo Wong           \tForth Tutorial"   s" http://www.albany.net/~hello/simple.htm"  "ViewWeb-Link ;
                MENUSEPARATOR
          false MENUMESSAGE     "Win32Forth"
                MENUSEPARATOR
                MENUITEM    "Dave Pochin        \tOnline Guide"          s" http://www.sunterr.demon.co.uk/guide.htm" "ViewWeb-Link ;
                MENUITEM    "Jeff Kelm          \tWin32API Examples"     s" http://www.concentric.net/~jkelm/"        "ViewWeb-Link ;
                MENUITEM    "Michael Hillerström\tDialog Editor"         s" http://users.cybercity.dk/~ccc27382/"     "ViewWeb-Link ;
                MENUITEM    "Marc Petremann     \tForth FAQs & Examples" s" http://perso.wanadoo.fr/mp7/forth/"       "ViewWeb-Link ;
                MENUSEPARATOR
          false MENUMESSAGE     "Macintosh"
                MENUSEPARATOR
                MENUITEM    "Michael Hore       \tJfar MOPS Page"       s" http://www.netaxs.com/~jayfar/mops.html"  "ViewWeb-Link ;
                ENDSUBMENU
        ...
        SUBMENU     "comp.lang.forth FAQs"
                MENUITEM    "General Information"       s" http://www.faqs.org/faqs/computer-lang/forth-faq/part1/"  "ViewWeb-Link ;
                MENUITEM    "Online Resources"          s" http://www.faqs.org/faqs/computer-lang/forth-faq/part2/"  "ViewWeb-Link ;
                MENUITEM    "Vendors & Authors"         s" http://www.faqs.org/faqs/computer-lang/forth-faq/part3/"  "ViewWeb-Link ;
                MENUITEM    "Forth Systems"             s" http://www.faqs.org/faqs/computer-lang/forth-faq/part4/"  "ViewWeb-Link ;
                MENUITEM    "Books & Periodicals"       s" http://www.faqs.org/faqs/computer-lang/forth-faq/part5/"  "ViewWeb-Link ;
                MENUITEM    "Groups & Organizations"    s" http://www.faqs.org/faqs/computer-lang/forth-faq/part6/"  "ViewWeb-Link ;
                ENDSUBMENU
        MENUSEPARATOR
        MENUSEPARATOR
        MENUITEM        "&About Win32Forth"                             about-Win32Forth ;
        MENUITEM        "&About WinView"                                about-demo ;
ENDBAR
...
:M Start:       ( -- )         \ create a new window object
                hWnd 0=
                IF      s" WinViewWindow" SetClassName: self \ set the class name
...
: wineditW-track ( -- )
                mousedown? 0= ?EXIT
                vdraging?
                IF      mousey drag-thick 2/ - Height drag-thick 2 + - -status 1 - 0max min
                        StartSize: WinViewToolbar nip max to drag-barV
                        set-console-height
...
: dbl-click-vdrag ( -- )
                ...
                StartSize: WinViewToolbar nip drag-thick + 2 + Height -status 0max between
                IF      StartSize: WinViewToolbar nip to drag-barV
                        ConsoleWindow to DocWindow
...
: wineditW-dblclick ( -- )       \ highlight the current word
                false to mousedown?
                mousey StartSize: WinViewToolbar nip < ?EXIT
                dbl-click-vdrag
                dbl-click-hdrag
                SetFocus: DocWindow
                Refresh: WinViewWindow ;
...
:M Classinit:   ( -- )
                ClassInit: super                \ init super class
                             3 to OriginX
                             3 to OriginY
                          self to WinViewWindow \ make myself the cur window
                 8                Width: sFont
                14               Height: sFont
                s" Courier" SetFaceName: sFont  \ default to Courier
                min-Edit-Tool-Bar  to WinViewToolbar
...
:M On_Init:     ( -- )          \ initialize the class
        ...
        then
        min-tool-bar?
        IF              min-Edit-Tool-Bar  to WinViewToolbar
        ELSE            max-Edit-Tool-Bar  to WinViewToolbar
        THEN
        tool-bar?
        IF              3    SetId: WinViewToolbar       \ then the next child window
                        self Start: WinViewToolbar       \ then startup child window
        THEN    edit-top  SetTopOf: EditWindow           \ set the edit window's top
...
:M Refresh:     ( -- )
                minimized? 0=
        IF      show-console?
                IF      Height console-height - StartSize: WinViewToolbar nip max
                ELSE    Height
                THEN    to drag-barV
...
:M StartToolBar: ( -- )
                tool-bar? 0=
                Floating: WinViewToolbar 0= and
                IF      TRUE to tool-bar?
                        GetSize: WinViewWindow nip 2/ to drag-barV
                        set-console-height
                        self Start: WinViewToolbar              \ then startup child window
                        LoadFindStrings
                        Refresh: self
                ELSE    tool-bar? 0=                            \ if flagged as not open
                        Floating: WinViewToolbar and            \ but flagged as floating
                        IF      SetFocus: WinViewToolbar        \ then bring to front
                        THEN
                THEN
                ;M
...
:M SwitchToolBar: ( -- )
                Floating: WinViewToolbar >r
                CloseToolBar: self
                WinViewToolbar max-Edit-Tool-Bar =
                IF      min-Edit-Tool-Bar  to WinViewToolbar
...
:M On_Paint:    ( -- )
                ...
                tool-bar?
                IF
                        0 0  StartSize: WinViewToolbar  Move: WinViewToolbar
                        LTGRAY_BRUSH Call GetStockObject
                        StartSize: WinViewToolbar drop 0        \ x,y origin
                        Width
                        StartSize: WinViewToolbar nip 1+ SetRect: EditRect
...
:M WinViewMessageBox:  ( szText szTitle style -- result )
...
: reflect-window { wparam \ theWindow -- WID }     \ return the ID of the reflected window
                window-list
...
:M ExWindowStyle: ( -- )
                ExWindowStyle: super
                WS_EX_ACCEPTFILES or            \ allow files to be dropped on WinView
...
:M WM_DROPFILES { hndl message wParam lParam \ cFiles drop$ -- res }
                ...
                RefreshOff: EditWindow
                FALSE defext_on? dup @ >r !     \ save, but reset the default extention addition
                cFiles 0
                ?DO     MAXCOUNTED
                        drop$ 1+ rel>abs
                        i
...
\                        THEN
                ...
                LOOP
                r> defext_on? !                 \ restore the default extention additon
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\       Automatic save of edit changes after a specifiable number of minutes
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
: update-highlight ( -- )
                line-cur mousey CharHeight: DocWindow / + file-lines 1- min to cursor-line
                cursor-line mlst =
                cursor-col  mcst > and
                cursor-line mlst > or
                IF      cursor-line to hled
                        cursor-col  to hced
                        mlst        to hlst
                        mcst        to hcst
...
: winedit-track { \ cursor-row -- }
                ...
                IF      BEGIN   cursor-col 1- 0max >col-cursor
                                WINPAUSE 20 ms
                                get-mouse-pos drop window-lmargin >=
                        UNTIL
                THEN
                FALSE to tracking? ;
...
: do-html-link  { \ html$ -- }
                ...
                IF      7 /string                                       \ remove "<A HREF"
                        ...
                        ELSE                    \ open a file
                                s" FILE:" HTML?
                                IF      2dup s" .EXE" caps-search nip nip       \ check for eecutable
                                        IF      "CLIP" html$ PLACE
                                                html$ +NULL
                                                html$ $EXEC
                                                IF      beep
                                                THEN
                                        ELSE    2dup '#' scan                   \ scan for string search marker
                                ...
                                ELSE                                    \ just a file, or sub web page
                                        ...
                                        ELSE    "+open-text             \ parse out and open the file
                                                browse? 0=
                                                IF      browse-toggle
                                                THEN
                                        THEN    html$ c@
...
\ Save and restore default settings for WinView
...
: save-defaults ( -- )
        ...
        IF      >E
                ...
                winview-web?                   s>d (d.) s" WebBrowse"      "SetDefault
                tool-bar? Floating: WinViewToolbar or
                                               s>d (d.) s" ToolBar"        "SetDefault
                Floating: WinViewToolbar       s>d (d.) s" FloatBar"       "SetDefault
                ...
                printed-columns 30 max 230 min s>d (d.) s" PrinterCols"    "SetDefault
           ...
           IF   GetWindowRect: FrameWindow
                ...
                console-height drag-thick max  s>d (d.) s" SplitHeight"    "SetDefault
                drag-barH                      s>d (d.) s" SelectWidth"    "SetDefault
           THEN GetFaceName: vFont                      s" WindowFont"     "SetDefault
...
: load-defaults ( -- )
                ...
                s" WinView\"       progreg   place      \ append the winview marker
                current-dir$ count progreg  +place      \ seperate defaults in each directory
                               ':' progreg c+place      \ seperate with a ';' char
                &prognam     count progreg  +place      \ and for each copy of the program
                progreg count '\' scan 1 /string        \ leave first '\' in the string
                begin   2dup  '\' scan                  \ change the rest to '/' characters
                while   '/' swap c!
                repeat  drop 2drop
                progreg ?+\
                s" SaveFind"       "GetDefault dup if 2dup number? 2drop 0<> to save-find?     then 2drop
                s" WebBrowse"      "GetDefault dup if 2dup number? 2drop 0<> to winview-web?   then 2drop
                s" ToolBar"        "GetDefault dup if 2dup number? 2drop 0<> to tool-bar?      then 2drop
                s" MinToolBar"     "GetDefault dup if 2dup number? 2drop 0<> to min-tool-bar?  then 2drop
                s" ForthWindow"    "GetDefault dup if 2dup number? 2drop 0<> to show-console?  then 2drop
                s" OpenPrevious"   "GetDefault dup if 2dup number? 2drop 0<> to open-previous? then 2drop
                s" PageMarks"      "GetDefault dup if 2dup number? 2drop 0<> to page-lines?    then 2drop
                s" PrintBorders"   "GetDefault dup if 2dup number? 2drop 0<> to border?        THEN 2drop
                s" CaseSensitive"  "GetDefault dup if 2dup number? 2drop 0<> to CaseSensitive? then 2drop
                s" SubDirectories" "GetDefault dup if 2dup number? 2drop 0<> to sub-dirs?      then 2drop
                s" AllOccurances"  "GetDefault dup if 2dup number? 2drop 0<> to all-occur?     then 2drop
                s" SaveMargin"     "GetDefault dup if 2dup number? 2drop 0<> to save-margin?   then 2drop
                s" Colorize"       "GetDefault dup if 2dup number? 2drop 0<> to colorize?      then 2drop
                s" CharHeight"     "GetDefault dup if 2dup number? 2drop     to char-height    then 2drop
                s" CharWidth"      "GetDefault dup if 2dup number? 2drop     to char-Width     then 2drop
                s" PrinterLPI"     "GetDefault dup if 2dup number? 2drop     to printer-lpi    then 2drop
                save-margin?
        IF      s" RightEdge"      "GetDefault dup if 2dup number? 2drop to def-right-edge     then 2drop
        THEN
                s" TabSize"        "GetDefault dup if 2dup number? 2drop to tab-size           THEN 2drop
                s" AutoSave"       "GetDefault dup if 2dup number? 2drop to save-minutes       then 2drop
                s" PrinterCols"    "GetDefault dup if 2dup number? 2drop to printed-columns    then 2drop
\ added new clipping to DEFEXT$, for new max length of 16 chars
                s" DefExt"         "GetDefault dup if 2dup DEFEXTMAX min defext$ place         THEN 2drop
                set-filter         
                s" SearchMask"     "GetDefault dup if 2dup "CLIP" mask-buf  place              THEN 2drop
                save-find?
        IF      s" SearchText"     "GetDefault dup if 2dup "CLIP" find-buf  place              THEN 2drop
        THEN    s" SearchPath"     "GetDefault dup if 2dup "CLIP" path-ptr  place              THEN 2drop
                s" WindowLeft"     "GetDefault number? 2drop
                s" WindowTop"      "GetDefault number? 2drop  SetOrigin: FrameWindow
                s" WindowHeight"   "GetDefault dup if 2dup number? 2drop to start-height       then 2drop
                s" WindowWidth"    "GetDefault dup if 2dup number? 2drop to start-width        THEN 2drop
                s" SplitHeight"    "GetDefault dup if 2dup number? 2drop to console-height     then 2drop
                s" SaveLines"      "GetDefault dup if 2dup number? 2drop to console-savelines  then 2drop
                s" SelectWidth"    "GetDefault dup if 2dup number? 2drop to drag-barH          THEN 2drop
                s" WindowFont"     "GetDefault dup
                IF      2dup SetFaceName: vFont
                THEN    2drop
                s" FloatBar"       "GetDefault dup if 2dup number? 2drop to floating-bar?      THEN 2drop
                ;
...
: VIEW-KEY-LOOP ( -- )
        BEGIN
                ...
                IF      TRUE to interpreting?
                        ...
                        command-args off
                THEN        
                BEGIN   s"  " StatusString count SetStatus: WinViewWindow
...
( >>> ) 'L'     +k_control              OF      load-active-file            ENDOF
        ...
        'R'     +k_control +k_shift     OF      WinViewWindow repeat-amacro ENDOF
        ...
        k_F1                            OF      s" STARTUP.TXT"  "browse    ENDOF
...
( >>> ) k_F12                           OF      save-and-load               ENDOF
        ...
        k_insert                        OF      ( ignore insert )           ENDOF
...
forth-msg-chain off     \ disable WinView's ability to recognize windows
...
with-img ' winedit turnkey winview           \ save WinView.EXE and WinView.IMG
